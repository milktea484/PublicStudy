...
class EmbeddingDataset(Dataset):
    def __init__(
        self, dataset_path, embeddings_path, reference): 
    ...
    def __getitem__(self, idx):
        ...
        if self.reference:
            seq_emb = torch.randn(self.embeddings[seq_id].shape[0], self.embeddings[seq_id].shape[1])
        else:
...
def create_dataloader(embeddings_path, partition_path, batch_size, shuffle, seed=None, reference=False, collate_fn=pad_batch):
