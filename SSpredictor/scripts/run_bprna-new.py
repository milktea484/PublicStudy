...
if args.weights_path:
    ...
    bprna_weigths_file = Path(f"{bprna_results_path}/weights.pth")
    if bprna_weigths_file.is_file():
        ...
        weight_path = f"results/{current_timestamp}/{dataset}_new/{llm}/weights"
        ...
        bprna_weigths_file_path = f"{out_path}/weigths"

print("+" * 80)
print(f"bpRNA-new TESTING STARTED".center(80))
print("+" * 80)
os.system(f"python src/test_model.py --emb {args.emb} --test_partition_path {data_path}test.csv --out_path {out_path} --weights_path {bprna_weigths_file_path} --seed {seed} --iter {iteration}")
